...
setwd("/data_genome1/MB208_WGS/analysis/All_mutations_reseq_mouse_2_and_3/")

all_samples_folder = "/data_genome1/MB208_WGS/analysis/All_mutations_reseq_mouse_2_and_3/"

all_vcf_raw = list()
# annotated variants, all samples
all_vcf_raw[["mouse2_3"]] = readVcf(paste(all_samples_folder, "mouse_2_3_wgs_calls_filtered.vcf.gz", sep=""))

all_samples = list()
all_samples[["mouse2_3"]] = c(paste("MOUlyuRAAA",c("B","C","E","F"),"AAA",sep=""))
...
all_anno = list()
all_anno[["mouse2_3"]] = fread("./Variant_annotation_mouse2_3.txt", sep="\t", header=T, stringsAsFactors = F)
...
save(all_variants, all_samples, anno_unique_sites, anno_unique_sites_highest_impact, file="Colibactin_WGS_Strelka_Calls_reseq_2_3.Rdata")
